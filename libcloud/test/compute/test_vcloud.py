...
class VCloud_1_5_MockHttp(MockHttp):
    ...
    def _api_query(self, method, url, body, headers):
        ...
        if 'type=user' in url:
            ...
            assert 'filter=(name==jrambo)' in url
            assert 'sortDesc=startDate'
    ...
    def _api_vApp_vapp_8c57a5b6_e61b_48ca_8a78_3b70ee65ef6b_action_controlAccess(self, method, url, body, headers):
        assert method == 'POST'
        assert '<IsSharedToEveryone>false</IsSharedToEveryone>' in body
        assert '<Subject href="https://vm-vcloud/api/admin/group/b8202c48-7151-4e61-9a6c-155474c7d413" />' in body
        assert '<AccessLevel>FullControl</AccessLevel>' in body
        body = self.fixtures.load('api_vApp_vapp_8c57a5b6_e61b_48ca_8a78_3b70ee65ef6a_controlAccess.xml')
        return httplib.OK, body, headers, httplib.responses[httplib.OK]
